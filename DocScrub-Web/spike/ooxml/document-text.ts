/**
 * Phase 2 OOXML spike -- targeted (not general-purpose) text extraction over
 * word/document.xml, with byte-offset tracking back to individual <w:t>
 * nodes. This is the structure DocumentRebuilder needs to know which run(s)
 * a candidate's text actually spans, which is the hard case named in
 ...
 * approach the existing Python docx_reader.py takes with lxml's
 * `.//w:t` xpath. A full DOM parse-and-reserialize is specifically *not*
 * what DocumentRebuilder should do (see rebuild.ts) -- reserializing an
 * OOXML DOM from scratch risks silently dropping or reordering attributes
 * Word depends on. Surgical byte-range replacement on the original string,
...
export interface RunText {
  /** Byte offsets, in the ORIGINAL document.xml string, of the text content
   *  between the <w:t...> and </w:t> tags (i.e. what to splice when
   *  replacing) -- not the tag boundaries themselves. */
  textStart: number;
  textEnd: number;
 ...
 * inside another <w:p> -- which is a real case, not a hypothetical: a text
 * box's txbxContent (or a content control's sdtContent) embeds its own
 * <w:p> inside a run that lives within an outer, "story-level" paragraph.
 * Found by evidence: an earlier version of this function assumed <w:p>
 * never nests (true for ordinary body/table/header/footer content) and
 * silently produced a corrupted, overlapping span for the outer paragraph
 * whenever a text box was present, which in turn corrupted redaction
 * splices badly enough that python-docx could no longer open the output.
 * See docs/ooxml-spike/phase-2-findings.md.
 *
 ...
 * (inside a text box or content control) is NOT returned as its own
 * separate ParagraphText -- but its <w:t> content is still included in
 * its containing top-level paragraph's flatText, because the run-scan
 * below operates over the whole (superset) span, matching
 * paragraph_text()'s `.//w:t` xpath, which is recursive regardless of
 * nesting. This is confirmed, not assumed: see phase-2-findings.md for the
 * text-box fixture that exercises exactly this.
 */
